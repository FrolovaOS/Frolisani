...
public class kitchen extends AppCompatActivity implements View.OnClickListener {
    ...
    String req ="SELECT * FROM app_recipes WHERE recipes_id In (SELECT r_k_id FROM app_recipes_kitchen WHERE app_recipes_kitchen.kitchen_name =?) AND recipes_block =?;";
    String data1 ;
    String data2 = "0";
    @Override
    ...
    public void onClick(View v) {
        switch (v.getId())
        {
            ...
            case R.id.german:
                ...
                request.add(data1);
                request.add(data2);
                intent.putStringArrayListExtra(EXTRA_REC1,request); startActivity(intent);
                break;
            case R.id.english:
                intent = new Intent(kitchen.this, Recicler_search.class);
                data1 = "Английская";
